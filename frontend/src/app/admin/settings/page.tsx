...
export default function SettingsPage() {
  ...
  const fetchBlobSettings = async () => {
    try {
      const response = await  api.fetch("/api/admin/blob-settings");
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchAdminEmails = async () => {
    try {
      const response = await  api.fetch("/api/admin/admin-emails");
      if (response.ok) {
        const data = await response.json();
        setAdminEmails(data);
      }
  ...
  const handleRemoveAdminEmail = async (email: string) => {
    ...
    try {
      const response = await  api.fetch(`/api/admin/admin-emails?email=${encodeURIComponent(email)}`, {
        method: "DELETE",
      });
